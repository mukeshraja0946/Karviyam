...
export default function MobileHomePage() {
  ...
  return (
    <div className="w-full bg-[#FAF7F2] pb-24 text-slate-900 select-none font-sans text-left min-h-screen">
      ...
      {/* ========================================================= */}
      {/* 1. MOBILE TOP HEADER (Cream/Warm Beige Reference Style)    */}
      {/* ========================================================= */}
      <header className="sticky top-0 z-40 bg-[#FAF4EC] border-b border-[#EBE1D3] px-3.5 pt-3 pb-2.5 shadow-2xs space-y-2.5">
        
        {/* Header Top Row: Logo & Action Icons */}
        <div className="flex items-center justify-between">
          ...
          >
            <div className="w-7 h-7 rounded-full bg-[#B71C1C] text-white flex items-center justify-center shadow-2xs">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
                <path d="M12 3C12 3 8.5 7.5 8.5 12C8.5 14.5 10 16.5 12 17.5C14 16.5 15.5 14.5 15.5 12C15.5 7.5 12 3 12 3Z" fill="currentColor"/>
                <path d="M12 17.5C9.5 16.8 6 14 6 11C6 9 7.5 7.5 7.5 7.5C7.5 7.5 5 10.5 5 13.5C5 16.5 8 18.5 12 19C16 18.5 19 16.5 19 13.5C19 10.5 16.5 7.5 16.5 7.5C16.5 7.5 18 9 18 11C18 14 14.5 16.8 12 17.5Z" fill="currentColor"/>
              </svg>
            </div>
            <div>
              <span className="font-serif font-black text-lg text-[#B71C1C] tracking-[0.15em] uppercase block leading-none">
                KARVIYAM
              </span>
              <span className="text-[7.5px] font-extrabold text-[#8C6D38] tracking-[0.18em] uppercase block mt-0.5 leading-none">
                WEAR YOUR STORY
              </span>
            </div>
          </div>

          {/* Right: Wishlist & Bag Icons */}
          <div className="flex items-center gap-2.5">
            {/* Wishlist Icon */}
            <button
              type="button"
              onClick={() => navigate('/wishlist')}
              className="w-9 h-9 rounded-full bg-white border border-[#EBE1D3] shadow-2xs flex items-center justify-center relative text-slate-800 hover:text-[#B71C1C] cursor-pointer active:scale-95 transition-transform"
              title="Wishlist"
            >
              <Heart className="w-5 h-5" />
              {activeWishlistCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-[#B71C1C] text-white text-[9px] font-black rounded-full h-4 min-w-[16px] px-1 flex items-center justify-center border border-white shadow-2xs">
                  {activeWishlistCount}
                </span>
              )}
            </button>
        ...
        <form onSubmit={handleSearchSubmit} className="relative w-full">
          <input
            ...
            placeholder="Search by brands, products, categories..."
            className="w-full bg-white border border-[#E5DACB] rounded-full py-2.5 pl-10 pr-4 text-xs font-medium text-slate-900 placeholder:text-slate-400 focus:outline-none focus:border-[#B71C1C] shadow-2xs transition-colors"
          />
