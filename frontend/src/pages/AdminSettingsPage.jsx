...
export default function AdminSettingsPage() {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xs space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 pb-3 gap-2">
                ...
                <button
                  ...
                  className="px-3.5 py-1.5 bg-slate-900 hover:bg-black text-white rounded-xl text-xs font-bold flex items-center gap-2 transition-all shadow-xs shrink-0 self-start sm:self-auto cursor-pointer"
                >
      ...
      {/* Maintenance Page Live Preview Modal */}
      {showMaintenancePreviewModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-5 bg-slate-900/80 backdrop-blur-xs">
          <div className="bg-white rounded-3xl max-w-5xl w-full max-h-[92vh] flex flex-col shadow-2xl border border-slate-100 overflow-hidden animate-in fade-in zoom-in duration-200">
            {/* Modal Header */}
            <div className="p-4 sm:p-5 border-b border-slate-100 flex flex-wrap items-center justify-between gap-3 bg-slate-900 text-white shrink-0">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-2xl bg-[#B71C1C] text-white flex items-center justify-center font-bold shrink-0">
                  <Wrench className="w-4 h-4" />
                </div>
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className="font-bold text-sm sm:text-base text-white">Maintenance Page Preview</h3>
                    <span className="px-2.5 py-0.5 bg-amber-500/20 text-amber-300 border border-amber-500/30 text-[10px] font-black uppercase tracking-wider rounded-full">
                      PREVIEW MODE
                    </span>
                  </div>
                  <p className="text-[11px] text-slate-400">Renders live using current unsaved form values without modifying production settings.</p>
                </div>
              </div>
            ...
            {/* Modal Footer */}
            <div className="p-4 border-t border-slate-100 bg-white flex flex-wrap items-center justify-between gap-3 shrink-0">
              <span className="text-[11px] font-medium text-slate-500">
                💡 Previewing current form values. Database and live customer storefront remain unchanged.
              </span>
              <button
                type="button"
                onClick={() => setShowMaintenancePreviewModal(false)}
                className="px-5 py-2.5 bg-slate-900 hover:bg-black text-white rounded-xl text-xs font-bold transition-all shadow-xs cursor-pointer"
              >
                Close Preview
              </button>
            </div>
