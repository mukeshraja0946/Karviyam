...
export default function AdminFooterPage() {
  const [activeTab, setActiveTab] = useState('general'); // 'general' | 'columns' | 'policies' | 'social' | 'newsletter'
    ...
    columns: [
      ...
      },
      {
        ...
        links: [
          ...
          { id: 'l12', title: 'Shop Catalogue', destinationType: 'Page', destination: '/shop', openNewTab: false, enabled: true, order: 2 },
  ...
  // Selected Column Index for Link Editing
  ...
  const handleOpenEditLinkModal = (linkIdx) => {
    ...
    setLinkForm({
      title: l.title || '',
      destinationType: l.destinationType || 'Category',
  ...
  return (
    <div className="p-4 sm:p-6 max-w-6xl mx-auto space-y-6 text-slate-800 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-200 shadow-xs">
        <div>
          ...
          <h1 className="text-xl sm:text-2xl font-display font-black text-slate-900 flex items-center gap-2.5 mt-1">
            ...
            <span>Footer Management CMS</span>
          </h1>
          <p className="text-xs text-slate-500 font-medium mt-1">
            Manage storefront footer branding, dynamic category links, customer care actions, policy popups, newsletter settings & social media handles.
          </p>
        </div>
      ...
      <div className="flex items-center gap-2 overflow-x-auto no-scrollbar border-b border-slate-200 pb-2">
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${
            activeTab === 'general' ? 'bg-[#B71C1C] text-white shadow-xs' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
          }`}
        >
          General & Logo
        </button>
        ...
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${
            activeTab === 'columns' ? 'bg-[#B71C1C] text-white shadow-xs' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
          }`}
        >
          Columns & Links ({formData.columns.length})
        </button>
        ...
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${
            activeTab === 'policies' ? 'bg-[#B71C1C] text-white shadow-xs' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
          }`}
        >
          Policy Popups & About Us
        </button>
        ...
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${
            activeTab === 'social' ? 'bg-[#B71C1C] text-white shadow-xs' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
          }`}
        >
          Social Media Links
        </button>
        ...
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${
            activeTab === 'newsletter' ? 'bg-[#B71C1C] text-white shadow-xs' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
          }`}
        >
          Newsletter Config
        </button>
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xs space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
              <div className="md:col-span-4 space-y-3 bg-slate-50 p-4 rounded-2xl border border-slate-200/80">
                <label className="block text-xs font-bold text-slate-800">Footer Logo Preview</label>
              ...
              <div className="md:col-span-8 space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-200 text-xs px-4 py-2.5 rounded-xl font-bold outline-none focus:border-[#B71C1C]"
                  />
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-800 mb-1">Brand Story & Description</label>
                  <textarea
                    rows={4}
                    value={formData.about}
                    onChange={(e) => setFormData(prev => ({ ...prev, about: e.target.value }))}
                    className="w-full bg-slate-50 border border-slate-200 text-xs p-3 rounded-xl font-medium outline-none focus:border-[#B71C1C] leading-relaxed"
                  />
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium mt-0.5">
                Manage Categories, Customer Care, and Quick Links columns. Bind category links directly to real category IDs (`/shop?category=...`).
              </p>
            </div>
            ...
            {!showAddColInput ? (
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <button type="button" onClick={handleAddColumn} className="bg-[#B71C1C] text-white font-bold text-xs px-3 py-2 rounded-xl cursor-pointer">Save</button>
                <button type="button" onClick={() => setShowAddColInput(false)} className="bg-slate-200 text-slate-700 font-bold text-xs px-3 py-2 rounded-xl cursor-pointer">Cancel</button>
              </div>
            )}
          </div>
            ...
            {formData.columns.map((col, idx) => (
              <button
                ...
                className={`px-4 py-2 rounded-2xl text-xs font-black uppercase tracking-wider flex items-center gap-2 transition-all cursor-pointer whitespace-nowrap ${
                  selectedColIdx === idx ? 'bg-[#B71C1C] text-white shadow-xs' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
                }`}
              >
                <span>{col.title}</span>
                <span className="bg-white/20 text-xs px-1.5 py-0.2 rounded-full font-bold">{(col.links || []).length}</span>
              </button>
            ))}
          </div>
          ...
          {activeColObj && (
            <div className="space-y-4 bg-slate-50/60 p-5 rounded-2xl border border-slate-200/80">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-4 rounded-xl border border-slate-200 shadow-2xs">
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className={`text-xs font-extrabold px-3 py-1 rounded-lg border cursor-pointer ${
                      activeColObj.enabled !== false ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-slate-100 border-slate-300 text-slate-600'
                    }`}
                  >
                ...
                <div className="flex items-center gap-2">
                  <button type="button" onClick={() => handleMoveColumn(selectedColIdx, 'up')} disabled={selectedColIdx === 0} className="p-1.5 bg-slate-100 hover:bg-slate-200 disabled:opacity-40 rounded-lg"><MoveUp className="w-4 h-4" /></button>
                  <button type="button" onClick={() => handleMoveColumn(selectedColIdx, 'down')} disabled={selectedColIdx === formData.columns.length - 1} className="p-1.5 bg-slate-100 hover:bg-slate-200 disabled:opacity-40 rounded-lg"><MoveDown className="w-4 h-4" /></button>
                  <button type="button" onClick={() => handleDeleteColumn(selectedColIdx)} className="p-1.5 bg-red-50 text-[#B71C1C] rounded-lg border border-red-200"><Trash2 className="w-4 h-4" /></button>
                  <button type="button" onClick={handleOpenAddLinkModal} className="bg-[#B71C1C] text-white font-extrabold text-xs px-4 py-2 rounded-xl flex items-center gap-1.5 cursor-pointer ml-2">
                ...
                <table className="w-full text-left text-xs border-collapse">
                  <thead>
                    <tr className="bg-slate-100 text-slate-700 font-extrabold uppercase text-[10.5px] tracking-wider border-b border-slate-200">
                      ...
                      <th className="p-3">Destination Path / Query</th>
                  ...
                  <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
                    {(!activeColObj.links || activeColObj.links.length === 0) ? (
                      <tr><td colSpan={6} className="p-6 text-center text-slate-400 italic">No links in this column. Click "+ ADD LINK" to add destination links.</td></tr>
                    ) : (
                      activeColObj.links.map((link, lIdx) => (
                        <tr key={link.id || lIdx} className="hover:bg-slate-50">
                          ...
                          <td className="p-3">
                            <span className="bg-slate-100 text-slate-700 px-2 py-0.5 rounded text-[10px] font-bold">
                              {link.destinationType || 'Category'}
                            </span>
                          </td>
                          <td className="p-3 font-mono text-[11px] text-sky-700 truncate max-w-[220px]">{link.destination}</td>
                          <td className="p-3">
                            <button type="button" onClick={() => handleToggleLink(lIdx)} className={`px-2 py-0.5 rounded text-[10px] font-black ${link.enabled !== false ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-200 text-slate-600'}`}>
                              {link.enabled !== false ? 'ON' : 'OFF'}
                            </button>
                          </td>
                          <td className="p-3 text-right">
                            <div className="flex items-center justify-end gap-1">
                              <button type="button" onClick={() => handleMoveLink(lIdx, 'up')} disabled={lIdx === 0} className="p-1 hover:bg-slate-200 disabled:opacity-30 rounded"><MoveUp className="w-3.5 h-3.5" /></button>
                              <button type="button" onClick={() => handleMoveLink(lIdx, 'down')} disabled={lIdx === activeColObj.links.length - 1} className="p-1 hover:bg-slate-200 disabled:opacity-30 rounded"><MoveDown className="w-3.5 h-3.5" /></button>
                              <button type="button" onClick={() => handleOpenEditLinkModal(lIdx)} className="p-1 hover:bg-sky-50 text-sky-600 rounded"><Edit2 className="w-3.5 h-3.5" /></button>
                              <button type="button" onClick={() => handleDeleteLink(lIdx)} className="p-1 hover:bg-red-50 text-[#B71C1C] rounded"><Trash2 className="w-3.5 h-3.5" /></button>
                            </div>
                          </td>
                        </tr>
                      ))
      ...
      {/* TAB 3: POLICY POPUPS & ABOUT US */}
      {activeTab === 'policies' && (
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xs space-y-6">
          <div className="border-b border-slate-100 pb-3">
            <h3 className="font-display font-bold text-base text-slate-900 flex items-center gap-2">
              <FileText className="w-5 h-5 text-[#B71C1C]" />
              <span>Policy Popups & About Us Content Editor</span>
            </h3>
            <p className="text-xs text-slate-500 font-medium mt-0.5">
              Edit formatted content for Return Policy, Terms of Service, Privacy Policy, and About Us. When clicked by customers, these popups open interactive modals displaying this live content.
            </p>
          </div>
          ...
          <div className="space-y-6">
            <div>
              <label className="block text-xs font-bold text-slate-800 mb-1 flex items-center gap-1.5">
                ...
                <span>Return & Replacement Policy Content (Modal Popup)</span>
              </label>
              <textarea
                rows={6}
                value={formData.returnPolicyContent}
                onChange={(e) => setFormData(prev => ({ ...prev, returnPolicyContent: e.target.value }))}
                placeholder="Return policy text or HTML formatting..."
                className="w-full bg-slate-50 border border-slate-200 text-xs p-3 rounded-xl font-mono leading-relaxed outline-none focus:border-[#B71C1C]"
              />
            </div>
            ...
            <div>
              <label className="block text-xs font-bold text-slate-800 mb-1 flex items-center gap-1.5">
                ...
                <span>Terms of Service Content (Modal Popup)</span>
              </label>
              <textarea
                rows={5}
                value={formData.termsContent}
                onChange={(e) => setFormData(prev => ({ ...prev, termsContent: e.target.value }))}
                placeholder="Terms of service text or HTML..."
                className="w-full bg-slate-50 border border-slate-200 text-xs p-3 rounded-xl font-mono leading-relaxed outline-none focus:border-[#B71C1C]"
              />
            </div>
            ...
            <div>
              <label className="block text-xs font-bold text-slate-800 mb-1 flex items-center gap-1.5">
                ...
                <span>Privacy Policy Content (Modal Popup)</span>
              </label>
              <textarea
                rows={5}
                value={formData.privacyContent}
                onChange={(e) => setFormData(prev => ({ ...prev, privacyContent: e.target.value }))}
            ...
            <div>
              <label className="block text-xs font-bold text-slate-800 mb-1 flex items-center gap-1.5">
                <Info className="w-4 h-4 text-[#B71C1C]" />
                <span>About KARVIYAM Story (Modal Popup)</span>
              </label>
              <textarea
                rows={5}
                value={formData.aboutUsContent}
                onChange={(e) => setFormData(prev => ({ ...prev, aboutUsContent: e.target.value }))}
                placeholder="About Us brand story text or HTML..."
                className="w-full bg-slate-50 border border-slate-200 text-xs p-3 rounded-xl font-mono leading-relaxed outline-none focus:border-[#B71C1C]"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full bg-slate-50 border border-slate-200 text-xs px-3.5 py-2.5 rounded-xl outline-none focus:border-[#B71C1C]"
              />
            </div>
      ...
      {/* TAB 5: NEWSLETTER CONFIG */}
      {activeTab === 'newsletter' && (
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xs space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <h3 className="font-display font-bold text-base text-slate-900 flex items-center gap-2">
              <Mail className="w-5 h-5 text-[#B71C1C]" />
              <span>Newsletter Configuration</span>
            </h3>
            ...
            >
              <span>Newsletter Section: {formData.newsletterEnabled ? 'ON' : 'OFF'}</span>
            </button>
          </div>
      ...
      {linkModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-4 border border-slate-200 text-left">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="font-display font-bold text-base text-slate-900">
                {editingLinkIndex >= 0 ? 'Edit Footer Link' : 'Add New Footer Link'}
              </h3>
              <button type="button" onClick={() => setLinkModalOpen(false)} className="text-slate-400 hover:text-slate-600 p-1">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSaveLinkModal} className="space-y-4 text-xs">
              ...
              <div>
                <label className="block font-bold text-slate-800 mb-1">Target Type</label>
                <select
                  ...
                  onChange={(e) => {
                    ...
                    else if (type === 'External URL') defaultDest = 'https://';
              ...
              <div className="flex gap-2 pt-3 border-t border-slate-100">
                <button type="button" onClick={() => setLinkModalOpen(false)} className="w-1/2 py-2.5 bg-slate-100 text-slate-700 font-bold rounded-xl">Cancel</button>
                <button type="submit" className="w-1/2 py-2.5 bg-[#B71C1C] text-white font-bold rounded-xl shadow-xs">Save Link</button>
              </div>
            </form>
          </div>
        </div>
