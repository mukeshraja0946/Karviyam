...
export default function MaintenancePage({ previewMode = false, previewSettings = null }) {
  ...
  return (
    <div className={`w-full flex flex-col items-center justify-center bg-[#F8FAFC] px-3 sm:px-4 select-none ${previewMode ? 'min-h-full flex-1 py-4 sm:py-6' : 'min-h-screen py-8 sm:py-12'}`}>
      <div className="w-full max-w-[460px] bg-white p-5 sm:p-11 rounded-[28px] sm:rounded-[36px] border border-gray-100/90 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.1)] text-center space-y-5 sm:space-y-6 mx-auto relative shrink-0">
        ...
        <div className="flex justify-center pt-1">
          <div className="flex items-center justify-center min-h-[90px] sm:min-h-[100px] bg-white w-full max-w-[360px] p-2">
            {resolvedLogoUrl && !logoFailed ? (
              <img 
                src={resolvedLogoUrl} 
                alt="Karviyam Logo" 
                className="h-24 sm:h-36 max-h-40 w-auto object-contain max-w-full transition-all" 
                onError={() => setLogoFailed(true)}
              />
            ) : (
              <div className="flex items-center gap-3 bg-red-50/70 px-5 sm:px-6 py-3.5 sm:py-4 rounded-3xl border border-red-100/80 shadow-2xs">
                <div className="w-10 sm:w-12 h-10 sm:h-12 rounded-2xl bg-[#B71C1C] text-white flex items-center justify-center font-black text-xl sm:text-2xl shadow-md shrink-0">
                ...
                <div className="text-left">
                  <span className="font-display font-black text-xl sm:text-2xl tracking-tight text-[#B71C1C] leading-none block">
                    KARVIYAM
                  </span>
